...
export default function Vasopressina() {
	...
	const [ConcentracaoAmpola, setConcentracaoAmpola] = useState(20);
	const [DoseMin] = useState(0.01);
	const [DoseMax] = useState(0.04);

	const ConcentracaoFinal = (ConcentracaoAmpola * VolumeDeDroga) / VolumeTotal;
	const DoseAtual = (mlhAtual * ConcentracaoFinal) / 60;

	const mlhMax = (DoseMax / ConcentracaoFinal) * 60;
	const mlhMin = (DoseMin / ConcentracaoFinal) * 60;

	const VolumeSoro = VolumeTotal - VolumeDeDroga;

	return (
		<div className={classes.root}>
			<span className={classes.droga}>Vasopressina</span> <br />
