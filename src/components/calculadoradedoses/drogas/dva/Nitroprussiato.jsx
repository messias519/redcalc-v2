...
export default function Nitroprussiato() {
	...
	const [ConcentracaoAmpola, setConcentracaoAmpola] = useState(25);
	const [DoseMin] = useState(0.25);
	const [DoseMax] = useState(10);

	const ConcentracaoFinal = (ConcentracaoAmpola * VolumeDeDroga) / VolumeTotal;
	const DoseAtual = ((mlhAtual * ConcentracaoFinal) / 60 / peso) * 1000;

	const mlhMax = (DoseMax * peso * 60) / 1000 / ConcentracaoFinal;
	const mlhMin = (DoseMin * peso * 60) / 1000 / ConcentracaoFinal;

	const VolumeSoro = VolumeTotal - VolumeDeDroga;

	return (
		<div className={classes.root}>
			<span className={classes.droga}>Nitroprussiato</span> <br />
